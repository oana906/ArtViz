...
from dsl import IsPerson, LabelOf, DefinitionOf, BirthDateOf, BirthPlaceOf, DeathDate, DeathPlace
...
class DeathDateQuestion(QuestionTemplate):

    regex = Lemmas("when do") + Person() + Lemma("die") +\
        Question(Pos("."))

    def interpret(self, match):
        birth_date = DeathDate(match.person)
        return birth_date, "age"
